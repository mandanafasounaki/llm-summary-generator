...
from src.models.schemas import DocumentClass, SummaryRequest, SummaryResponse, SummaryCompareReq
...
class TestChatModelHandler:
        ...
        class MockChatModelHandler:
            ...
            def get_completion(self, provider: str, prompt: str) -> str:
                ...
                try:
                    ...
                    raise RuntimeError(f"Failed to get completion from '{provider}'.") from e
        
        return MockChatModelHandler()

    @patch("tenacity.nap.time.sleep", return_value=None)  # Avoid actual sleep during retry
    def test_get_completion_success(self, _, chat_handler):
        chat_handler.models[self.PROVIDER].invoke.return_value = MagicMock(content=self.RESPONSE)
    ...
    def test_get_completion_invalid_provider(self, _, chat_handler):
        with pytest.raises(ValueError, match="The provider 'invalid_provider' is not supported."):
